...
<section
  ...
  class="relative z-[1000] p-5 bg-transparent"
    ...
    <a href="<?php echo esc_url(home_url('/')); ?>" class="flex btn">
      <?php if ($logo_url) : ?>
        <div class="max-w-[124px]">
          <img src="<?php echo esc_url($logo_url); ?>" alt="<?php echo esc_attr($logo_alt); ?>" class="w-full h-full object-contain"/>
        </div>
      <?php endif; ?>
    </a>
  </div>
      ...
      <ul id="primary-menu" class="flex gap-1 items-center text-sm font-bold text-sky-800">
        <?php foreach ($primary_navigation->toArray() as $index => $item) : ?>
          <li
            ...
            <?php endif; ?>
          >
            <div class="flex items-center py-2 pr-3 pl-3.5 rounded-[100px]">
              <a
                href="<?php echo esc_url($item->url); ?>"
                class="text-sky-800 hover:text-sky-900 btn <?php echo $item->active ? 'font-semibold' : ''; ?>"
              >
  ...
  <div class="hidden lg:flex shrink-0 gap-4 items-center">
    ...
    <div class="w-12 h-12 flex items-center justify-center border border-gray-300 rounded-full">
      <button @click="searchOpen = true">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none">
          <path d="M14 14L11.1 11.1M12.6 7.3A5.3 5.3 0 1 1 2 7.3a5.3 5.3 0 0 1 10.6 0Z"
            stroke="#00628F" stroke-width="2" stroke-linecap="round"/>
        </svg>
      </button>
    </div>
    ...
    <div class="flex gap-2 items-center text-sm font-bold">
      ...
      <a
        ...
        class="flex items-center gap-2 px-4 py-3 border border-sky-800 rounded-full hover:bg-sky-50 transition"
      >
        <?php if ($donate_icon) : ?>
          <img src="<?php echo esc_url($donate_icon['url']); ?>" class="w-4"/>
        <?php endif; ?>
      ...
      <?php if ($contact_button) : ?>
        <a 
          ...
          class="flex items-center gap-2 px-4 py-3 text-white font-bold rounded-[100px] border-[3px] border-[#1C959B] bg-[#008BCC] hover:opacity-90 transition"
        >
...
>
  <div class="bg-white p-4 rounded-lg w-full max-w-[800px]">
    <form method="get" action="<?php echo esc_url(home_url('/')); ?>" class="flex gap-2">
      ...
      />
      <button class="btn-primary px-5">Search</button>
    </form>
  </div>
</div>
